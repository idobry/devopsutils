            ...
            steps
            {
                script
                {
                    withCredentials([usernamePassword(credentialsId: 'idobry_github', usernameVariable: 'username', passwordVariable: 'password')])
                    {
                        ...
                        sh "git commit -am 'update to version ${SOURCE_BRANCH}-${env.BUILD_ID}' && git push http://$username:$password@github.com/idobry/devopsutils.git origin/${SOURCE_BRANCH} HEAD:${SOURCE_BRANCH}"
                    }
/*
                    dir('.devopsutils')
                    {
